...
function EditorDoc() {
  ...
  const [selectedDocument, setSelectedDocument] = useState(() => {
    try {
      const generatedDocument = JSON.parse(sessionStorage.getItem("snapTask.generatedDocument") || "null");
      if (generatedDocument?.text) {
        sessionStorage.removeItem("snapTask.generatedDocument");
  ...
  const saveDocument = () => {
    ...
    const nextDocuments = [
      savedDocument,
      ...savedDocuments.filter((document) => document.id !== savedDocument.id),
    ];
  ...
  const sendToFlashcards = () => {
    if (!selectedDocument?.text) return;
    sessionStorage.setItem("snapTask.flashcardsSource", JSON.stringify({
  ...
  const improveDocument = async ({ prompt }) => {
    ...
    if (pendingImageRef.current) {
      formData.append("imagem", pendingImageRef.current);
    }
    ...
    try {
      ...
      if (!response.ok || data.erro || !data.texto) {
        throw new Error(data.erro || "Não foi possível melhorar o documento.");
      }
  ...
  return (
    <div className="min-h-screen bg-[#05070d] text-white flex items-center justify-center font-[system-ui,-apple-system,'Segoe_UI',sans-serif]">
      ...
      <div className={`fixed top-1/2 -translate-y-1/2 w-55 bg-[rgba(20,20,30,0.96)] backdrop-blur-xl border-l-2 border-[#FFD700] rounded-l-[14px] overflow-hidden shadow-[-8px_0_30px_rgba(0,0,0,0.45)] transition-all duration-300 z-999 ${menuOpen ? "right-14" : "-right-55"}`}>
        <div className="flex flex-col py-2">
          <div className="text-[#FFD700] text-lg font-extrabold tracking-[1px] px-4 py-3 border-b border-[rgba(255,215,0,0.25)] mb-2">DEV NAV</div>
          {menuItems.map((item) => (
            <a key={item.label} href={item.path} className="no-underline">
              <div className="flex items-center gap-3 px-4 py-3 transition cursor-pointer hover:bg-[rgba(255,215,0,0.08)]">
                ...
                <span className="text-[#f1f1f1] text-sm font-medium">{item.label}</span>
              </div>
            </a>
          ))}
        </div>
      ...
      <div className="flex flex-col items-center p-5">
        ...
        <div className="text-center mb-5">
          <h1 className="text-[32px] font-bold bg-linear-to-br from-[#FFD700] to-[#FFA500] bg-clip-text text-transparent">JOVI Camera</h1>
          <p className="text-[#7e7e8a] mt-1.25 text-sm">Experiência de câmera intuitiva + SnapTask para processamento inteligente</p>
        </div>
          ...
          {!selectedDocument ? (
            <div className="w-full h-full flex flex-col">
              <div className="flex items-center gap-3.5 mt-5 mb-5.5">
                ...
                <div>
                  <h2 className="text-lg text-white font-bold">Editor Doc</h2>
                  <span className="text-[#8e8e9e] text-[10px]">Notas geradas a partir de fotos de lousa</span>
                </div>
              </div>
              <div className="flex items-center gap-2 mb-3.5">
                <span className="text-[#777d8f] text-[10px] tracking-[2px] font-semibold">ARQUIVOS RECENTES</span>
              </div>
              <div className="flex flex-col gap-3 overflow-y-auto pr-1">
                ...
                {[...savedDocuments, ...documents].map((document, index) => (
                  <button key={index} onClick={() => openDocument(document)} className="w-full flex items-center gap-3.5 bg-[#10131c] border border-[#232632] rounded-[18px] p-3.5 cursor-pointer transition text-left hover:bg-[#171b26]">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm mb-0.75 font-semibold truncate">{document.title}</h3>
                      <small className="text-[#8e8e9e] text-[10px]">{document.date}</small>
                      <p className="text-[#777d8f] text-[11px] mt-1.25 overflow-hidden whitespace-nowrap text-ellipsis">{document.preview}</p>
                    </div>
                    <div className="text-[#777] text-lg">›</div>
                  </button>
                ))}
            ...
            <div className="w-full h-full flex flex-col">
              <div className="flex items-center gap-3.5 mt-5 mb-5.5">
                ...
                <div>
                  <h2 className="text-lg text-white font-bold">{selectedDocument.title}</h2>
                  <span className="text-[#8e8e9e] text-[10px]">{selectedDocument.date}</span>
                </div>
              </div>
              <div className="h-40 rounded-[18px] overflow-hidden border border-[#232632] relative mb-3.5">
                <div className={`w-full h-full flex items-center justify-center transition-all duration-700 relative ${isProcessed ? "bg-linear-to-br from-[#2a2a35] to-[#16161d]" : "bg-linear-to-br from-[#1e1e28] to-[#121218]"}`}>
                  <div className={`absolute w-[78%] h-[78%] rounded-xl transition-all duration-700 ${isProcessed ? "bg-[#f4f4f4] rotate-0" : "bg-[#d9d9d9] skew-y-2 skew-x-1"}`} />
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-[rgba(0,0,0,0.75)] backdrop-blur-md flex items-center justify-center gap-2.5 z-20">
                    ...
                    <span className="text-[#aaa] text-[11px]">Corrigindo...</span>
                  </div>
                )}
              </div>
              <div className="flex gap-2.5 mb-3.5">
                <button onClick={processDocument} disabled={isProcessing} className="flex-1 bg-[#ffc400] text-black font-semibold rounded-[14px] p-3 flex items-center justify-center gap-2 cursor-pointer transition hover:opacity-90 disabled:opacity-50">
                  <span>{isProcessed ? "Reprocessar" : "Corrigir Perspectiva"}</span>
                </button>
              ...
              <div className="flex items-center justify-between mb-2.5">
                <span className="text-[10px] tracking-[2px] text-[#777d8f] font-semibold">TEXTO EXTRAÍDO</span>
                <div className="flex items-center gap-1.25 bg-[#3793ff20] text-[#3793ff] px-2 py-1.25 rounded-full text-[10px] font-semibold">
                  <span>Editável</span>
                </div>
              </div>
              <textarea value={selectedDocument.text} onChange={(event) => setSelectedDocument((prev) => ({ ...prev, text: event.target.value }))} className="flex-1 bg-[#10131c] border border-[#232632] rounded-[18px] p-4 text-white resize-none outline-none text-xs leading-[1.7] font-mono focus:border-[#ffc400]" />
              {transcriptionError && (
                <p className="text-red-400 text-xs mt-2">{transcriptionError}</p>
              )}
              {improvementComment && (
                <div className="mt-3 p-3 rounded-xl bg-[#ffc40012] border border-[#ffc40040] text-[#c9c3a6] text-xs leading-relaxed">
                  <strong className="text-[#ffc400]">O que foi melhorado:</strong>{" "}
                  {improvementComment}
                </div>
              )}
